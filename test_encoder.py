...
def test_encoder():
  ...
  obs_size = 100
  def sample(k,*, max_num_mixtures=6, dims=2):
    ...
    # mean = (jnp.arange(max_num_mixtures)[b]/max_num_mixtures).reshape(-1,1)
    mean = (jnp.stack([jnp.arange(max_num_mixtures)]*dims,axis=1)/max_num_mixtures)[b].reshape(-1,dims)
    s += mean
  ...
  def loss(m, obs, b, key):
    assert obs.ndim == 3 and obs.shape[2]==2 and jnp.dtype(b) == jnp.int32 and b.ndim == 1
